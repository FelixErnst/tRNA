...
these features for further analysis. The package expects a `GRanges` object with 
certain columns as input. The following columns are a requirement:
`tRNA_length`, `tRNA_type`, `tRNA_anticodon`, `tRNA_seq`, `tRNA_str`,
`tRNA_CCA.end`.
...
mitochondrial tRNA and other tRNAs missing certain features. However, for finge
...
The results can also be accessed using `gettRNASummary`.
